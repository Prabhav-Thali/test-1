...
"ubuntu-18.04" | "ubuntu-20.04")
        ...
        sudo apt-get install -y bison flex wget build-essential git gcc make zlib1g-dev libreadline-dev patch |& tee -a "$LOG_FILE"
...
"rhel-7.6" | "rhel-7.7" | "rhel-7.8"| "rhel-8.1" |"rhel-8.2" )
        ...
        sudo yum install -y git wget gcc gcc-c++ make readline-devel zlib-devel bison flex glibc-langpack-en procps-ng diffutils patch |& tee -a "$LOG_FILE"
        else
        sudo yum install -y git wget build-essential gcc gcc-c++ make readline-devel zlib-devel bison flex patch |& tee -a "$LOG_FILE"
        fi
...
"sles-12.5" | "sles-15.1" | "sles-15.2")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- 'Installing the dependencies for postgresql from repository \n' |& tee -a "$LOG_FILE"
        sudo zypper install -y git gcc gcc-c++ make readline-devel zlib-devel bison flex gawk patch |& tee -a "$LOG_FILE"
        configureAndInstall |& tee -a "$LOG_FILE"
        ;;
