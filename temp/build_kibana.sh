...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Kibana/7.10.0/build_kibana.sh
...
function configureAndInstall() {
    ...
    wget https://raw.github.ibm.com/vitthalb/opensource-porting-s390x/kibana_7.10.0/Kibana/scripts/7.10.0/patch/kibana_patch.diff?token=AABCPXLQKDU6AEHC6G2FFWC72N2W4
    mv kibana_patch.diff?token=AABCPXLQKDU6AEHC6G2FFWC72N2W4 kibana_patch.diff
    git apply kibana_patch.diff
    
    if [[ "${DISTRO}" == "rhel-7.8" ]] || [[ "${DISTRO}" == "rhel-7.9" ]] ; then
    #wget $PATCH_URL/register_git_hook.diff
    wget https://raw.github.ibm.com/vitthalb/opensource-porting-s390x/kibana_7.10.0/Kibana/scripts/7.10.0/patch/register_git_hook.diff?token=AABCPXLQ53FEP24H2FEJEGK72N2XA
    mv register_git_hook.diff?token=AABCPXLQ53FEP24H2FEJEGK72N2XA register_git_hook.diff
