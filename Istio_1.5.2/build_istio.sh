...
"ubuntu-16.04" | "ubuntu-18.04" | "ubuntu-19.10")
	...
	printf -- '\nInstalling dependencies \n' |& tee -a "$LOG_FILE"
	sudo apt-get update
	sudo DEBIAN_FRONTEND=noninteractive apt-get install -y pkg-config zip tar zlib1g-dev unzip git vim tar wget automake autoconf libtool make curl libcurl3-dev bzip2 mercurial patch clang-format-9 clang-9 llvm-9 llvm-9-dev libc++abi1-9 libc++1-9
	dependencyInstall
	buildHelm
