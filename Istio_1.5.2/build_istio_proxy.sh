...
CURDIR="$(pwd)"
REPO_URL="https://raw.githubusercontent.com/vibhutisawant/test/master/Istio_1.5.2"
...
function buildGO() {
	...
	if command -p "go" version | grep 1.13 >/dev/null; then
		...
		cd "${CURDIR}"		
		wget https://storage.googleapis.com/golang/go1.14.linux-s390x.tar.gz
		tar -xzf go1.14.linux-s390x.tar.gz
  ...
  rm -rf tools/docker/BUILD
	...
	if [ -f "$PROXY_DEBUG_BIN_PATH/envoy" ]; then
       ...
       curl -o Makefile.core.mk.patch $REPO_URL/Makefile.core.mk.patch
        patch "${SOURCE_ROOT}/proxy/Makefile.core.mk" Makefile.core.mk.patch
        curl -o envoy.bazelrc.patch $REPO_URL/envoy.bazelrc.patch
        patch "${SOURCE_ROOT}/proxy/envoy.bazelrc" envoy.bazelrc.patch
		fi
...
"ubuntu-18.04")
	...
	printf -- '\nInstalling dependencies \n' |& tee -a "$LOG_FILE"
	sudo apt-get update
	sudo DEBIAN_FRONTEND=noninteractive apt-get install -y git pkg-config zip zlib1g-dev unzip python3 libtool automake cmake curl wget build-essential rsync gcc-7 g++-7 libgtk2.0-0 ninja-build clang-format-9 clang-9 llvm-9 llvm-9-dev libc++abi1-9 libc++1-9
