...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Puppet/6.5.02/build_puppet.sh
...
function checkPrequisites() {
	...
	if [[ "$DISTRO" == "rhel-6.x" && "$USEAS" == "server"  ]]; then	
	   printf -- " puppetserver is not supported on %s \n"  "$DISTRO" |& tee -a "$LOG_FILE"
	   exit 
        fi
	if [[ "$FORCE" == "true" ]]; then
...
function configureAndInstall() {
	...
	if [[ "$ID" == "ubuntu" && ( "$VERSION_ID" == "18.04" || "$VERSION_ID" == "19.04"  ) ]]; then
		cd "$CURDIR"
...
"ubuntu-19.04")
	...
	printf -- 'Installing the dependencies for puppet from repository \n' |& tee -a "$LOG_FILE"
	sudo apt-get update >/dev/null
	sudo apt-get install -y g++  libreadline-dev tar make git wget libsqlite3-dev libc6-dev cron locales unzip libyaml-dev zlibc zlib1g-dev zlib1g libxml2-dev libgdbm-dev libssl-dev ant zip |& tee -a "$LOG_FILE"
	configureAndInstall |& tee -a "$LOG_FILE"
	;;
...
"sles-12.4" | "sles-15" | "sles-15")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- 'Installing the dependencies for puppet from repository \n' |& tee -a "$LOG_FILE"
	if [[ "$DISTRO" ==  "sles-12.4" ]]; then	
	  sudo zypper install  gcc-c++ readline-devel tar openssl unzip libopenssl-devel make git wget sqlite-devel glibc-locale cron net-tools curl ant zip|& tee -a "$LOG_FILE"
        else 
	  sudo zypper install -y gcc-c++ readline-devel tar openssl unzip libopenssl-devel make git wget sqlite-devel glibc-locale cron net-tools curl ant zip|& tee -a "$LOG_FILE"
        fi
